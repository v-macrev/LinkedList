...
void LinkedList<T>::insertOrdered(const T& newData) {
    ...
    // If the list is empty or the new node data is less than or equal to the data of the current head
    if (!head_ || newData <= head_->data) {
        newNode->next = head_;
        if (head_) {
            head_->prev = newNode;
        } else {
...
while (leftPtr && rightPtr) {
    // Compare the data of the nodes and insert the smaller one into the merged list
